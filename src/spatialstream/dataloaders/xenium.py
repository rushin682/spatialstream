...
class XeniumDataset(Dataset):
    ...
    def _validate(
        ...
        for region_name in self.regions:
            ...
            if table_name is not None:
                ...
                # check that the regions specified are the elements annotated by the table.
    ...
    def _return_function(
        ...
        tile = XeniumDataset._ensure_single_scale(tile)

        if return_annot is not None:
            return_annot = [return_annot] if isinstance(return_annot, str) else return_annot

            gexp = dataset_table[idx, :].X.toarray() if issparse(dataset_table.X) else dataset_table[idx, :].X
            coord = dataset_table[idx, :].obsm["spatial"]

            if np.all([i in dataset_table.obs for i in return_annot]):
                annotations = dataset_table.obs[return_annot].iloc[idx].values.reshape(1, -1)
            else:
                raise ValueError(
                    f"If `return_annot` is a `str`, it must be a column name in the table. "
                    f"If it is a `list` of `str`, each element should be as above, and they should all be entirely in obs. "
                    f"Got {return_annot}."
                )

            return {"image": tile, "gexp": gexp, "coord": coord, "annotations": annotations}
            ...
            return SpatialData(
                images={dataset_index.iloc[idx][XeniumDataset.IMAGE_KEY]: tile},
    ...
    def _get_return(
        ...
        return partial(
            XeniumDataset._return_function,
    ...
    def __getitem__(self, idx: int) -> Any | SpatialData:
        ...
        image = self.sdata[row[self.IMAGE_KEY]]
...
if __name__ == "__main__":
    ...
    zarr_path = Path("/Users/rushin.gindra/Documents/LaminConsult/mock_data/zarr_files")
    zarr_file = "Xenium_V1_hKidney_cancer_section.zarr"
    sdata = sd.read_zarr(zarr_path / zarr_file)

    def my_transform(batch: dict) -> dict[str, torch.Tensor]:
        """
        Transform function to convert batch dictionary values to torch tensors.

        Args:
            batch: Dictionary containing 'image', 'gexp', 'coord', and 'annotations'

        Returns
        -------
            Dictionary with the same keys but values converted to torch tensors
        """
        # Convert xarray DataArray to tensor
        tile = torch.tensor(batch["image"].values, dtype=torch.float32)

        # Convert gene expression (pandas row) to tensor
        gexp = torch.tensor(batch["gexp"], dtype=torch.float32)

        # Convert coordinate tuple to tensor
        coord = torch.tensor(batch["coord"], dtype=torch.float32)

        # Convert annotations (pandas row) to tensor
        # Use appropriate dtype based on annotation type (float for continuous, long for categorical)
        annotations = torch.tensor(batch["annotations"], dtype=torch.float32)
    ...
    for i, batch in enumerate(dataloader):
        ...
        print(f"  Gene expression shape: {batch['gexp'].shape}")
        print(f"  Annotations: {batch['annotations'].shape}")
